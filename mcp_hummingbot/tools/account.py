...
# Tool definition is now automatically generated by FastMCP from the Pydantic model


async def get_account_state(account_names: Optional[List[str]], connector_names: Optional[List[str]]) -> Dict[str, Any]:
    """Get a comprehensive account state including balances, positions, and orders"""
    client = await hummingbot_client.get_client()
...
async def _fetch_exchange_state(client, account: str, exchange: str) -> Dict[str, Any]:
    """Fetch state for a single exchange"""
    # Fetch balances, positions, and orders in parallel
    balances_task = client.trading.get_balances(
        account_name=account,
        exchange=exchange
    )
    positions_task = client.trading.get_positions(
        account_name=account,
        connectors=[exchange]
    )
    orders_task = client.trading.get_orders(
        account_name=account,
        connectors=[exchange],
...
get_account_state.tool_definition = Tool(
